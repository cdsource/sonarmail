...
public class DailyPackageRpt
{
	...
	private void DoDailyRpt(int iType)
	{
		...
		String strSelectDate = DateTimeUtil.GetDay("yyyy-MM-dd", 0);

		try
		{
			String strDevHtmlString = FileUtil.GetFileTxt("file_coverage.html");
			...
			String strSql = "select * from (SELECT t1.long_name"
					...
					+ " AND t3.metric_id IN (37,43,46,48,50) GROUP BY t1.id)t1 where   branch_coverage<25 ORDER BY t1.long_name";
