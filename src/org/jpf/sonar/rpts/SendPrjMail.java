...
public class SendPrjMail
{
	...
	public SendPrjMail()
	{
		...
		try
		{
			strSelectDate = DateTimeUtil.getToday();
			logger.info("strSelectDate=" + strSelectDate);
			strDevHtmlString = FileUtil.GetFileTxt("prj.html");
			MakePrjAvgInfo();
	...
	private void MakeModuleMail() throws Exception
	{
		...
		try
		{
			...
			//SonarMail.sendMail("", newMailString, "GBK", " SONAR产品质量日报(自动发出)");
			
			SonarMail.sendMail(
							"BSS-Billing-PRD-MNG,chenyf,caiwei,zhanghy4,chenzj,fanxj,wangjing,kangcl,zhangqd,chencm,huangyd,lizl2,shengjun,chongst,zhanglk,qiaowen,wangning3",
